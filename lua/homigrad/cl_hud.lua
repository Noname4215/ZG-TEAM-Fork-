...
local function DrawTextCentered(text, font, x, y, color)
	...
	draw.DrawText(text, font, x, y - totalHeight / 2, color, TEXT_ALIGN_CENTER)
end

local function DrawRingSlice(cx, cy, radius, innerRadius, startDeg, endDeg, color, outlineColor)
	local steps = math.max(2, math.ceil(math.abs(endDeg - startDeg) / 5))
	
	surface.SetDrawColor(color)
	surface.SetMaterial(matHuy) -- Use the white material
	
	for i = 1, steps do
		local a1 = math.rad(Lerp((i-1)/steps, startDeg, endDeg))
		local a2 = math.rad(Lerp(i/steps, startDeg, endDeg))
		
		-- Adjust for screen coordinates: 0 is Right, 90 is Down.
		-- We want standard math usage where possible, but sin/cos works as:
		-- x = cos(a), y = sin(a) -> 0 is Right, 90 is Down.
		
		local c1, s1 = math.cos(a1), math.sin(a1)
		local c2, s2 = math.cos(a2), math.sin(a2)
		
		local p1 = {x = cx + c1 * radius, y = cy + s1 * radius, u=0, v=0}
		local p2 = {x = cx + c2 * radius, y = cy + s2 * radius, u=0, v=0}
		local p3 = {x = cx + c2 * innerRadius, y = cy + s2 * innerRadius, u=0, v=0}
		local p4 = {x = cx + c1 * innerRadius, y = cy + s1 * innerRadius, u=0, v=0}
	...
	if outlineColor then
		...
		local aStart = math.rad(startDeg)
		local cs, ss = math.cos(aStart), math.sin(aStart)
		surface.DrawLine(cx + cs * innerRadius, cy + ss * innerRadius, cx + cs * radius, cy + ss * radius)
		
		local aEnd = math.rad(endDeg)
		local ce, se = math.cos(aEnd), math.sin(aEnd)
		surface.DrawLine(cx + ce * innerRadius, cy + se * innerRadius, cx + ce * radius, cy + se * radius)
		...
		for i = 1, steps do
			...
			surface.DrawLine(cx + c1 * radius, cy + s1 * radius, cx + c2 * radius, cy + s2 * radius)
			surface.DrawLine(cx + c1 * innerRadius, cy + s1 * innerRadius, cx + c2 * innerRadius, cy + s2 * innerRadius)
		end
	end
end
...
local function CreateRadialMenu(options_arg)
	...
	menuPanel.Paint = function(self, w, h)
		...
		-- Angle: 0 at Right (1,0) in standard math.
		-- GMod Screen: Y is Down.
		-- atan2(y, x) -> 0 is Right, 90 is Down, 180 is Left, -90 is Top.
		local ang = math.deg(math.atan2(y, x))
		ang = math.NormalizeAngle(ang)
		-- We want 0-360 range for easier logic
		...
		local radius = ScrH() * (options_arg ~= nil and 0.4 or 0.45) * viewLerp
		local innerRadius = radius * 0.3
		local sliceDeg = 360 / count
		local gap = 2
		local dist = math.sqrt(x*x + y*y)
		
		isMouseOnRadial = dist <= radius and dist > innerRadius
		...
		-- Start angle offset to make the first item centered at TOP.
		-- Top is -90 degrees (or 270).
		-- Item 1 spans [start, end]. Center should be -90.
		-- Center = start + sliceDeg/2 = -90
		-- start = -90 - sliceDeg/2
		local globalOffset = -90 - (sliceDeg / 2)
		
		for num, option in ipairs(options) do
			local startDeg = globalOffset + (num - 1) * sliceDeg
			local endDeg = globalOffset + num * sliceDeg
			
			-- Normalize angles for hit testing
			-- We need to check if 'ang' (0-360) is between startDeg and endDeg
			-- Normalize startDeg/endDeg to 0-360 range logic for comparison? 
			-- Easier: Rotate mouse angle by -globalOffset to align with 0-based index.
			
			local checkAng = ang - globalOffset
			checkAng = math.NormalizeAngle(checkAng)
			if checkAng < 0 then checkAng = checkAng + 360 end
			
			-- Now checkAng is relative to the start of Item 1 (which is at 0 in this relative space)
			-- Item 1: 0 to sliceDeg
			-- Item 2: sliceDeg to 2*sliceDeg
			
			local relativeStart = (num - 1) * sliceDeg
			local relativeEnd = num * sliceDeg
			
			local isHover = isMouseOnRadial and (checkAng >= relativeStart and checkAng < relativeEnd)
			...
			optionSelected[num] = LerpFT(0.1, optionSelected[num], isHover and 1 or 0)
			
			local sD = startDeg + gap
			local eD = endDeg - gap
			
			local colBase = Color(30, 30, 30, 200)
			local colHigh = Color(180, 60, 60, 220)
			local colDraw = Color(
				Lerp(optionSelected[num], colBase.r, colHigh.r),
				Lerp(optionSelected[num], colBase.g, colHigh.g),
				Lerp(optionSelected[num], colBase.b, colHigh.b),
				Lerp(optionSelected[num], colBase.a, colHigh.a)
			)
			
			local animR = radius * (1 + 0.02 * optionSelected[num])
			
			if option[3] then
				local subOpts = option[4]
				local subCount = #subOpts
				local ringSize = (animR - innerRadius) / subCount
				
				local ringHover = -1
				if isHover then
					local d = dist - innerRadius
					ringHover = math.ceil(d / ringSize)
					if ringHover < 1 then ringHover = 1 end
					if ringHover > subCount then ringHover = subCount end
					current_option_select = ringHover
				end
				
				for i, opt in pairs(subOpts) do
					local rOuter = innerRadius + ringSize * i
					local rInner = innerRadius + ringSize * (i - 1)
					
					local isRingSel = (isHover and current_option_select == i)
					local rCol = isRingSel and Color(180, 60, 60, 240) or Color(40, 40, 40, 200)
					
					DrawRingSlice(w/2, h/2, rOuter, rInner, sD, eD, rCol, Color(155, 0, 0, 150))
					
					local midAng = math.rad(Lerp(0.5, sD, eD))
					local midR = (rOuter + rInner) / 2
					local tx = w/2 + math.cos(midAng) * midR
					local ty = h/2 + math.sin(midAng) * midR
					DrawTextCentered(opt, "HomigradFontSmall", tx, ty, Color(255,255,255))
				end
			else
				DrawRingSlice(w/2, h/2, animR, innerRadius, sD, eD, colDraw, Color(155, 0, 0, 150))
				
				local midAng = math.rad(Lerp(0.5, sD, eD))
				local midR = innerRadius + (animR - innerRadius) * 0.6
				local tx = w/2 + math.cos(midAng) * midR
				local ty = h/2 + math.sin(midAng) * midR
				...
				DrawTextCentered(txt or "", "HomigradFont", tx, ty, Color(255,255,255))
			end
		end
	end
end
