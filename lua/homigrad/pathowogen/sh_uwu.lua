...
properties.Add( "furrify", {
	MenuLabel = "Furrify :3", -- Name to edisplay on the context menu
	...
	Receive = function( self, length, ply ) -- The action to perform upon using the property ( Serverside )
		...
		hg.Furrify(ent)
		hg.AdminLogs.Log(ply, "furrify", ent)
	end
} )
...
if CLIENT then
	...
	hook.Add("RenderScreenspaceEffects", "furbot", function()
		...
		elseif damageSum >= 2 then
			Status = "Незначительный урон"
		elseif damageSum > 0 then
			Status = "Минимальный урон"
		end

		DrawTextAndShadow("Статус системы: " .. Status, "ZB_ProotOSMedium", sw * 0.01, sh * 0.96, bluewhite, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
