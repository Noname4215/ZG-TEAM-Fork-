...
function CLASS.Off(self)
	...
	self:SetNWInt("SpeedGainClassMul", 1)
...
local alpha = 0
local hintAlphaLerp = 0
local pawsHintAlphaLerp = 0
...
function CLASS.HUDPaint(self)
	...
	for _, ply in player.Iterator() do
		if ply != lply and ply.PlayerClassName != "furry" and ply:GetPos():Distance(lply:GetPos()) < 70 then
			targetAlpha = 255
			break
		end
	end

	hintAlphaLerp = Lerp(FrameTime() * 5, hintAlphaLerp, targetAlpha)
	...
	-- Hint for infecting
	surface.SetFont("ZB_ProotOSMedium")
	surface.SetTextColor(50, 50, 50, hintAlphaLerp)
	local hintTxt = "Схвати регдолл человека и зажми левую кнопку мыши для заражения"
	local w2, h2 = surface.GetTextSize(hintTxt)
	surface.SetTextPos(sw * 0.5 - w2 * 0.5, sh * 0.8 - h2 - ScreenScale(5))
	surface.DrawText(hintTxt)

	surface.SetFont("ZB_ProotOSMedium")
	surface.SetTextColor(164, 171, 235, hintAlphaLerp)
	local hintTxt = "Схвати регдолл человека и зажми левую кнопку мыши для заражения"
	local w2, h2 = surface.GetTextSize(hintTxt)
	surface.SetTextPos(sw * 0.5 - w2 * 0.5, sh * 0.8 - h2 - ScreenScale(5))
	surface.DrawText(hintTxt)

	-- Hint for removing paws from combat state
	local activeWep = lply:GetActiveWeapon()
	local pawsHintTargetAlpha = 0
	if IsValid(activeWep) then
		local class = activeWep:GetClass()
		if (class == "weapon_hg_coolhands" or class == "weapon_hands_sh") and activeWep.GetFists and activeWep:GetFists() then
			pawsHintTargetAlpha = 255
		end
	end

	pawsHintAlphaLerp = Lerp(FrameTime() * 3 , pawsHintAlphaLerp, pawsHintTargetAlpha)

	surface.SetFont("ZB_ProotOSMedium")
	surface.SetTextColor(50, 50, 50, pawsHintAlphaLerp)
	local pawsHintTxt = "Нажми R, чтобы убрать лапы из боевого состояния"
	local w3, h3 = surface.GetTextSize(pawsHintTxt)
	surface.SetTextPos(sw * 0.5 - w3 * 0.5, sh * 0.85 - h3 - ScreenScale(5))
	surface.DrawText(pawsHintTxt)

	surface.SetFont("ZB_ProotOSMedium")
	surface.SetTextColor(164, 171, 235, pawsHintAlphaLerp)
	local pawsHintTxt = "Нажми R, чтобы убрать лапы из боевого состояния"
	local w3, h3 = surface.GetTextSize(pawsHintTxt)
	surface.SetTextPos(sw * 0.5 - w3 * 0.5, sh * 0.85 - h3 - ScreenScale(5))
	surface.DrawText(pawsHintTxt)
