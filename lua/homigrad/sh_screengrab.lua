...
local function ScreenGrab(ply, target, file_name)
	...
	-- Отслеживание для античита
	target.AC_ScreengrabStartTime = CurTime()
	target.AC_BlockedScreengrabNets = 0
...
if(SERVER)then
	...
	net.Receive( "ScreengrabInitCallback", function( _, ply )
		...
		-- Сбрасываем счетчики блокировок для античита
		if ply.AC_ScreengrabStartTime then
			ply.AC_ScreengrabStartTime = nil
			ply.AC_BlockedScreengrabNets = 0
			ply.AC_RenderCaptureBlockDetections = 0
		end
