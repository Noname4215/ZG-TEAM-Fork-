...
function MODE:SpawnDeltaSquad(count)
	...
	net.Start("zb_commandertransmit")
		net.WriteString("Но не забывайте: вы здесь для того, чтобы эвакуировать выживших, которые все еще находятся в этой зоне. Даже не пытайтесь вернуться на базу, не забрав хотя бы одного из них.")
...
function MODE:SpawnSquadHelicopter()
	...
	if #heliPoints > 0 then
		self:CreateTimer("UWUCopter", math.random(240, 300), 1, function()
			...
			if #squad == 0 then
				self:BroadcastCommander("Извините, эвакуация не состоится, Вы обречены. Лучше убейте себя...")
			else
				self:BroadcastCommander("Эвакуационный вертолет уже на вашей территории. Найдите его, и подайте как нибудь сигнал")
	...
	if spawnPoint then
		self:CreateTimer("CQExtract", math.random(120, 180), 1, function()
			...
			if #squad == 0 then
				...
				self:BroadcastCommander("Дельта сквад уже на подходе. Найдите его, и они приведут вас к безопасному местуМ.")
...
MODE.LootTable = {
	...
	{16, {
		...
		{0.5,"weapon_pm9"},
		{0.5,"weapon_tec9"},
		{0.5,"weapon_revolver2"},
		--{0.06,"weapon_revolver357"},
		{0.7,"weapon_deagle"},
		{0.7,"weapon_colt9mm"},
		{0.7,"weapon_doublebarrel_short"},
		--{7,"ent_armor_vest6"},
		{1,"ent_armor_vest7"},
		{2, "ent_armor_helmet2"},
	...
	{8, {
		{0.70,"weapon_doublebarrel"},
		{0.60,"weapon_remington870"},

		{0.70,"weapon_glock18c"},
		{0.70,"weapon_skorpion"},
		{0.70,"weapon_mac11"},
		{0.70,"weapon_uzi"},
		{0.70,"weapon_tmp"},

		{1,"weapon_hg_molotov_tpik"},
		{1,"weapon_hg_pipebomb_tpik"},

		{0.70,"weapon_kar98"},
		{0.70,"weapon_ar_pistol"},
		{0.70,"weapon_draco"},
		{0.70,"weapon_mp5"},
		--{0.03,"weapon_xm1014"},
...
function MODE:CanPlayerEnterVehicle(ply, ent) -- damdn i forgot about the broken mode hooks lmao
	if ply.PlayerClassName == "furry" then
		if (ply.cantdrivecd or 0) < CurTime() then
			ply:Notify("Эээм, Я не знаю как этим управлять.", 0, "idkdrive", 0)
...
function MODE:SpawnGlideHelicopter()
		...
		if extracted then
			for _, v in ipairs(players) do
				...
				if !self.saved.ContractorEscapee then
					...
					self:BroadcastContractor("Кто-то сбежал! Твоя миссия провалена! Твой контракт разорван.")
				end
			end
			...
			timer.Simple(5, function()
				self:BroadcastCommander("Эвакуационный вертолет вылетел из этой территории. Извини приятель, Думаю ты остался позади.")
			end)
		end
	end
	...
	heli:CallOnRemove("CommanderReply", function()
		...
		timer.Simple(5, function()
			self:BroadcastCommander("Я потерял связь с эвакуационным вертолетом... Сука...")
		end)
	end)
...
function MODE:ZB_LootMultiplier(ply)
	...
	if ply.PlayerClassName == "furry" then
		return #MODE.humans * 0.5
	else
		return #MODE.furries * 0.2
	end
end
