			...
			if (IsValid(ply) and istable(result) and #result > 0 and result[1].value) then
                ...
                if zb.GuiltSQL.PlayerInstances[steamID64].value < 0 then
                    ...
                    timer.Simple(0, function()
                        ply:Ban(5, false, "Your karma is too low: " .. math.Round( karma, 0 ) .. ". Try again in 5 minutes." )
...
hook.Add("HomigradDamage", "GuiltReg", function(ply, dmgInfo, hitgroup, ent, harm) 
    ...
    Attacker.Karma = math.Clamp((Attacker.Karma or 100) - add * math.max(((1 - (zb.GuiltTable[Victim][Attacker] or 0)) / 1),0), -5, zb.MaxKarma)
    ...
    if shouldBanGuilt and Attacker.Guilt >= 100 then
        ...
        if ULib then
            ULib.ban( Attacker, 30, "Kicked and banned for dealing too much team damage." )
        else
    ...
    if Attacker.Karma <= -5 then
        ...
        timer.Create("simplewaitforkarmadrop"..Attacker:EntIndex(), 0, 1, function()
            if IsValid(Attacker) then -- if the player haven't left in that exact tick then we do him dirty
                ...
                Attacker:Ban(30, true, "Your karma is too low: " .. math.Round(karma, 0) .. ". Try again in 30 minutes.")

                PrintMessage(HUD_PRINTTALK, "Player "..name.." has been banned for 30 minutes for karma.")
            end
        end)
    end
end)
