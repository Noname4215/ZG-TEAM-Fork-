...
COMMANDS.bigmap = {
	function(ply, args)
		if not zb.CanChangeMode(ply) then ply:ChatPrint("You don't have access") return end
...
net.Receive("ZB_RequestRoundList", function(len, ply)
	if IsValid(ply) and zb.CanChangeMode(ply) then
		zb.SendModesInfoToClient(ply)
		zb.SendRoundListToClient(ply)
	end
end)
...
net.Receive("ZB_UpdateRoundList", function(len, ply)
	if not IsValid(ply) or not zb.CanChangeMode(ply) then return end
...
concommand.Add("zb_checkchances",function(ply) if zb.CanChangeMode(ply) then zb.CheckChances() end end)
concommand.Add("zb_rerollchances",function(ply) if zb.CanChangeMode(ply) then zb.RerollChances() zb.CheckChances() end end)
...
net.Receive("AdminSetGameMode", function(len, ply)
	if not zb.CanChangeMode(ply) then return end
...
if SERVER then
	...
	net.Receive("AdminSetGameMode", function(len, ply)
		...
		if !zb.CanChangeMode(ply) and not zb.modes[modeKey]:CanLaunch() then
